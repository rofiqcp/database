...
Complete API endpoint documentation for the Redis Learning Module backend.
...
- Uses `SMEMBERS` to get all IDs from the `items:index` set
- Uses `MGET` to retrieve all item JSON strings in a single operation
...
**Implementation Details:**
- Uses `GET item:{id}` to retrieve the JSON string
- Parses JSON and returns the item object
...
**Implementation Details:**
- Uses `INCR item:counter` to generate auto-incrementing ID
- Uses `SET item:{id}` to store the item as a JSON string
- Uses `SADD items:index` to add the ID to the index set
...
**Implementation Details:**
- Retrieves existing item with `GET item:{id}`
- Preserves `created_at` from original item
- Updates `updated_at` to current timestamp
- Overwrites key with `SET item:{id}`
...
**Implementation Details:**
- Uses `DEL item:{id}` to remove the key
- Uses `SREM items:index` to remove the ID from the index set
...
**Implementation Details:**
- Retrieves all items using `SMEMBERS` + `MGET`
- Filters in-memory (Redis doesn't have built-in text search for this pattern)
- Case-insensitive text matching on name and description
- Exact match on category
- Numeric comparison for price range
