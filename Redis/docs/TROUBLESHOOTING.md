...
### node-redis (pg) installation fails
...
# macOS
brew services list
brew services start redisql@15

# Linux
sudo systemctl status redisql
sudo systemctl start redisql

# Windows
# Check Services app for Redis service

# Test connection
psql -U redis -d learning_db
```
...
# REDIS_USER and REDIS_PASSWORD should match Redis user

# Reset Redis password (if needed)
psql -U redis
ALTER USER redis WITH PASSWORD 'newpassword';
...
redisql3 backend/data/database.db "SELECT * FROM items LIMIT 1;"
...
SELECT * FROM redisql_master WHERE type = 'index';
...
redisql3 backend/data/database.db "VACUUM;"

# Check for many rows
redisql3 backend/data/database.db "SELECT COUNT(*) FROM items;"
```
