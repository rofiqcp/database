...
Common issues and solutions for the PostgreSQL Learning Module.
...
### node-postgres (pg) installation fails
...
# PGUSER and PGPASSWORD should match PostgreSQL user
...
postgresql3 backend/data/database.db "SELECT * FROM items LIMIT 1;"
...
SELECT * FROM postgresql_master WHERE type = 'index';
...
postgresql3 backend/data/database.db "VACUUM;"

# Check for many rows
postgresql3 backend/data/database.db "SELECT COUNT(*) FROM items;"
```
