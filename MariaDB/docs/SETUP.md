# MariaDB Learning Module - Setup Guide

Complete step-by-step setup instructions for the MariaDB learning module.
...
mysql --version # Should show MariaDB 10.5+ or MySQL compatible client
```
...
# Or navigate to the MariaDB folder if already downloaded
```
...
**Important**: Update `DB_USER` and `DB_PASSWORD` with your MariaDB credentials.
...
Expected output:
```
🚀 MariaDB Backend Server running on http://localhost:3000
📊 Database: MariaDB
...
# Then from MariaDB root directory
...
- You should see the MariaDB Learning Module interface
...
If you encounter MariaDB connection issues:
...
2. Check [DATABASE_INFO.md](DATABASE_INFO.md) for MariaDB-specific information
...
Congratulations! Your MariaDB Learning Module is ready to use! 🎉
