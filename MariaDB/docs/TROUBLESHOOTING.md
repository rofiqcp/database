...
Common issues and solutions for the MariaDB Learning Module.
...
### mariadb driver installation fails
...
# Windows
# Check Services app for MariaDB service
...
# DB_USER and DB_PASSWORD should match MariaDB user
...
// Make sure all queries use try/finally with conn.release()
```
...
**Solutions:**
```bash
# Connect to MariaDB and check tables
mysql -u root -p learning_db -e "SHOW TABLES;"

# Drop and recreate table if needed
mysql -u root -p learning_db -e "DROP TABLE IF EXISTS items;"
...
# Verify data exists
mysql -u root -p learning_db -e "SELECT * FROM items;"
