...
**Request:**
```bash
curl http://localhost:3000/api/data/abc123xyz
```
...
**Request:**
```bash
curl -X PUT http://localhost:3000/api/data/def456uvw \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Gaming Mouse",
...
**Request:**
```bash
curl -X DELETE http://localhost:3000/api/data/def456uvw
```
...
### 6. POST /api/search
Search and filter items using Elasticsearch full-text search with fuzzy matching.
...
```json
{
  "query": "string (full-text search in name/description with fuzzy matching)",
  "category": "string (exact match on keyword field)",
  "minPrice": "number",
  "maxPrice": "number"
}
```
...
Elasticsearch generates string-based document IDs automatically (e.g., `"abc123xyz"`). Unlike relational databases, IDs are not sequential integers.
...
// Get single item
const item = await axios.get('/api/data/abc123xyz')
...
// Update item
const updated = await axios.put('/api/data/abc123xyz', {
  name: 'Updated Name',
...
// Delete item
await axios.delete('/api/data/abc123xyz')
