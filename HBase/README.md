...
Complete full-stack CRUD application built with Node.js, Express, Vue 3, and Apache HBase database.

![](https://img.shields.io/badge/Database-HBase-blue)
...
This is a comprehensive learning module demonstrating full-stack development with Apache HBase database. It includes a complete backend API with an HBase column-family emulator, modern frontend UI, and extensive documentation covering HBase architecture and concepts.
...
- ✅ **HBase Column Families** - Emulates wide-column storage model
...
- **Database**: Apache HBase (emulated for learning)
...
- Wide-column NoSQL database concepts
- HBase architecture (RegionServer, HMaster, ZooKeeper)
...
HBase is ideal for:
- **Big Data**: Billions of rows across distributed clusters
- **Real-time Analytics**: Sub-millisecond reads for known keys
- **Time-Series Data**: IoT sensor data, metrics, logs
- **Social Media**: User feeds, messaging, activity streams
- **Content Management**: Large-scale content storage
- **Fraud Detection**: Real-time pattern analysis

## ⚠️ Limitations

- This module uses an in-memory HBase emulator (data resets on restart)
- Real HBase requires Hadoop/HDFS infrastructure
- Not ideal for small datasets or simple CRUD applications
- No native SQL support (use Apache Phoenix for SQL layer)
- Minimum 2GB RAM for development, 16-64GB for production

For simpler use cases, consider PostgreSQL or MongoDB.
...
# Data reset (expected - in-memory emulator)
# Restart backend and re-add data via API or frontend
```
