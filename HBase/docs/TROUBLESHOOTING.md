...
- Items created but lost

**This is expected behavior.** The HBase emulator uses in-memory storage for learning purposes. Data resets on each server restart.

**Solutions:**
- This is by design for the learning module
- Add sample data after each restart via API or frontend
- For persistent storage, connect to a real HBase cluster
...
- Put/Get operation failures

**Solutions:**
```bash
# Check database.js initialization
node --check backend/src/database.js

# Verify table creation in database.js
# Should create 'items' table with ['info', 'meta'] families
...
**Symptoms:**
- Error: "Column family X not found in table Y"

**Solutions:**
- Verify the column family name matches defined families
- Check database.js: `this.createTable('items', ['info', 'meta'])`
- Only 'info' and 'meta' column families are defined
...
**Solutions:**
- The in-memory emulator should be very fast
- Check if too many items are stored (scan returns all)
