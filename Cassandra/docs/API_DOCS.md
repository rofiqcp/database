...
**Request:**
```bash
curl -X PUT http://localhost:3000/api/data/6ba7b810-9dad-11d1-80b4-00c04fd430c8 \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Gaming Mouse",
...
**Request:**
```bash
curl -X DELETE http://localhost:3000/api/data/6ba7b810-9dad-11d1-80b4-00c04fd430c8
```
...
```json
{
  ...
  "category": "string (exact match, uses secondary index)",
  "minPrice": "number",
  "maxPrice": "number"
}
```
...
**Note:** Category filtering uses a secondary index on the `category` column. Text search and price range filtering are performed client-side. For production, consider using materialized views or Apache Solr integration.
...
- Validated with regex: `/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i`
...
3. **Requests**:
   ...
   - GET Single Item: `{{baseUrl}}/data/<uuid>`
   - POST Create: `{{baseUrl}}/data`
   - PUT Update: `{{baseUrl}}/data/<uuid>`
   - DELETE Item: `{{baseUrl}}/data/<uuid>`
