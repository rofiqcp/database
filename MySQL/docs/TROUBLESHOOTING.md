...
GRANT ALL PRIVILEGES ON learning_db.* TO 'myuser'@'localhost';
FLUSH PRIVILEGES;
```
...
**Symptoms:**
- "Unknown database 'learning_db'" error
...
const pool = mysql.createPool({
  connectionLimit: 50,  // Increase from default 20
  // ... other config
});

// Or increase MySQL max connections
// In my.cnf or my.ini:
// max_connections = 200
```
...
**Symptoms:**
- "Table 'learning_db.items' doesn't exist" error
- Queries fail

**Solutions:**
```bash
# Check if table exists
mysql -u root -p learning_db -e "SHOW TABLES"

# Manually create the table if needed
mysql -u root -p learning_db -e "
...
mysql -u root -p learning_db -e "SELECT COUNT(*) FROM items"
...
mysql -u root -p learning_db -e "SHOW TABLE STATUS LIKE 'items'"
```
...
- Emoji not saving properly

**Solutions:**
```bash
# Check database charset
mysql -u root -p -e "SELECT DEFAULT_CHARACTER_SET_NAME FROM information_schema.SCHEMATA WHERE SCHEMA_NAME = 'learning_db'"

# Fix charset if needed
mysql -u root -p -e "ALTER DATABASE learning_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci"

# Fix table charset
mysql -u root -p learning_db -e "ALTER TABLE items CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci"
```
...
mysql -u root -p learning_db -e "SELECT * FROM items LIMIT 1"
...
# Check table size
mysql -u root -p learning_db -e "SELECT table_name, ROUND(data_length/1024/1024, 2) AS 'Data (MB)' FROM information_schema.tables WHERE table_schema = 'learning_db'"
```
...
# Grant permissions
mysql -u root -p
GRANT ALL PRIVILEGES ON learning_db.* TO 'youruser'@'localhost';
FLUSH PRIVILEGES;
```
