...
Common issues and solutions for the CouchDB Learning Module.
...
### nano package installation fails
...
- Backend fails to initialize database
...
# If installed as a service (Linux)
...
cat backend/.env | grep COUCHDB_URL

# Test authentication
curl http://admin:password@localhost:5984

# If running in Admin Party mode (no admin set)
# Access Fauxton to set up admin
...
**Symptoms:**
- "unauthorized" error when creating database
- Permission denied

**Solutions:**
```bash
# Ensure admin credentials are in COUCHDB_URL
...
**Symptoms:**
- "conflict" error on startup
- Views not working
- Design document update fails

**Solutions:**
```bash
# Delete and recreate design document
curl -X GET http://admin:password@localhost:5984/learning_db/_design/items
# Note the _rev value

curl -X DELETE http://admin:password@localhost:5984/learning_db/_design/items?rev=<_rev>
...
**Symptoms:**
- "Document update conflict" (409) error
- Update operations fail
- Stale revision errors

**Solutions:**
```javascript
// Always get the latest revision before updating
const doc = await db.get(docId);
doc.name = 'Updated Name';
await db.insert(doc);  // Uses latest _rev from doc

// If conflict persists, fetch and retry
try {
  await db.insert(updatedDoc);
} catch (err) {
  if (err.statusCode === 409) {
    const latest = await db.get(updatedDoc._id);
    updatedDoc._rev = latest._rev;
    await db.insert(updatedDoc);
  }
}
```
...
# Verify CouchDB data directory
# Check CouchDB config for database_dir
curl http://admin:password@localhost:5984/_node/_local/_config/couchdb/database_dir
```
...
# Check database size
curl http://admin:password@localhost:5984/learning_db | python3 -m json.tool
```
...
### Document not found (404) on update/delete

**Symptoms:**
- Update returns 404
- Delete returns 404
- Document exists but can't be found

**Solutions:**
```bash
# CouchDB uses string IDs (UUIDs), not integers
# Make sure you're using the correct document ID

# Check if document exists
curl http://admin:password@localhost:5984/learning_db/<document_id>

# Ensure the document has type: 'item'
# The API filters by type field
```
...
curl -X POST http://admin:password@localhost:5984/learning_db/_index \
  -H "Content-Type: application/json" \
  -d '{"index": {"fields": ["type", "created_at"]}, "name": "idx-type-created"}'
...
# In the API, this is handled automatically
# by fetching the document before updating
```
...
4. **Test Individually**
   - Test CouchDB with curl
   - Test backend with curl
   - Test frontend independently
   - Check database via Fauxton
...
7. **Use Fauxton** - CouchDB's web UI helps debug data issues
...
- Verify CouchDB is accessible via Fauxton (http://localhost:5984/_utils)
